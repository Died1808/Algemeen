...
Vkl = Polygon([(7,1.05),(4.1,1.4),(7,1.4),(30,1.4),(30,0.70),(10,0.70)])
V = Polygon([(1.0,0.30),(3,0.1),(3,0.01), (30,0.01),(30,0.70),(10,0.70),(4.92,0.3)])
Los= Polygon([(0.01,1.2),(0.1,1.2),(0.2,1.0),(0.5,0.5),(1,0.3),(3,0.1),(3,0.01), (0.01,0.01)])
#
...
#        
    ...
    def _parseDataLine(self, line):
        ...
        if len(line.split()) == 0:
            return
        z  = round(abs(float(args[self._columns[1]])),4)
        dz = round(self.z - z,4)
        ...
        try:
            pw = round(float(args[self._columns[3]]),4) 
            if pw < 1e-3:
                pw=0.003
            elif pw>20:
...
##########        
        ...
        if ke <=0:
            ...
            fact=1.7
            kb  = (qc / ke)*0.875*fact #Als deze *1.5 gaat daalt de c
            self.kb.append(kb)
...
#
        fig, ax = plt.subplots(nrows=1, ncols=2, gridspec_kw = {'width_ratios':[2.6, 0.4]})
...
#
        ...
        df.plot(x='Rg',   y='depth', ylim = (ymin, ymax), xlim = (0,1), yticks=range (ymin, ymax, ds), xticks=range (0, 1, 1), xerr=1.0, elinewidth=0.5,  ax=ax[1], legend=False, color='navajowhite');
        ...
        df.plot(x='Xg',   y='depth', ylim = (ymin, ymax), xlim = (0,1), yticks=range (ymin, ymax, ds), xticks=range (0, 1, 1), xerr=1.0, elinewidth=0.5,  ax=ax[1], legend=False, color='orange');
        ...
        df.plot(x='Og',   y='depth', ylim = (ymin, ymax), xlim = (0,1), yticks=range (ymin, ymax, ds), xticks=range (0, 1, 1), xerr=1.0, elinewidth=0.5,  ax=ax[1], legend=False, color='peru');
        ...
        df.plot(x='Ol',   y='depth', ylim = (ymin, ymax), xlim = (0,1), yticks=range (ymin, ymax, ds), xticks=range (0, 1, 1), xerr=1.0, elinewidth=0.5,  ax=ax[1], legend=False, color='darkseagreen');
...
#
        ...
        leg=plt.legend(['Kwartair zand, sortering = goed',
                        '      Dekzand (Boxtel) ',
                        'Kwartair zandige klei tot leem',
                        'Kwartair zand, sortering = matig',
                        '',
                        'Kwartair zandige klei tot leem',
                        'Kwartair zand, sortering = slecht',
                        '',
                        'Kwartair zandige klei tot leem', 
                        'Klei',
                        'Grof Boxtel, Matig grof Diestien, Peelo-afzettingen',
                        'Boxtel, Matig fijn Diestien',
                        'Lemig Boxtel, lemig Diestien, Potklei, Keileem, T_fijn',
                        'Grof Glauconietzand, Grof Diestien',
                        'Glauconietzand, Diestienzand, Scheldeafzettingen',
                        'Geconsolideerde klei, Boomse klei, Topklei Tegelen (T3)', 
    ...
    bbox_to_anchor=(8, 1), loc='upper left',fontsize=10, frameon=1, title='T (Bodemtype)', title_fontsize=14);
